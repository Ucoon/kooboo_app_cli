...
class ConsoleSelector<T> {
  ...
  ConsoleSelectorResult<T> choose() {
    ...
    return ConsoleSelectorResult(
      result.index,
      _options[result.index ?? 0],
  ...
  _SimpleResult _chooseAnsi() {
    ...
    while (result == null) {
      ...
      for (var i = 0; i < _options.length; i += 1) {
        ...
        if (i == currentIndex) {
          _stdout.write(green(i == currentIndex ? '> ' : '  '));
          _stdout.write(green('$humanIndex'.padLeft(1)));
          _stdout.write(green(') '));
          _stdout.writeln(green(_sanitizeLength(_options[i].toString())));
        } else {
          _stdout.write(white(i == currentIndex ? '> ' : '  '));
          _stdout.write(white('$humanIndex'.padLeft(1)));
          _stdout.write(white(') '));
          _stdout.writeln(white(_sanitizeLength(_options[i].toString())));
        }
      }
